...
paquetes <- c("tidyverse","cluster","factoextra","NbClust","tidyr","dplyr","ggalt","ggrepel")
ipak(paquetes)
```
...
```
Al realizar las pruebas de normalidad podemos notar como las siguientes variables no se distribuyen normalmente:
...
Como todas las variables caracterizan al pokémon en cuestión, se realizaran análisis no paramétricos.
...
Notemos apriori tres grupos haciendo un heatmap de distancias.
...
Notemos que de los 3 clusters generados, uno de ellos esta conformado por Mewtwo, Mega Mewtwo X , Mega Mewtwo Y , como habiamos mencionado al principio, no solo a priori es considerado como de los mas fuertes a modo de creencia, si no que sus estadisticas dentro del videojuego son mayores a comparacion a los demas grupos teniendo un Cluster propio.

El cluster verde podemos catalogarlos como pokemon equilibrados en cuanto a sus estadisticas y el azul como a los pokemon mas debiles de esta muestra.
...
Con este endograma notemos como se ordenan apartir de las estadisticas de los Pokemon, haciendo uso de los clusters generados que van desde los mas fuertes a los mas debiles.
...
Notemos que los 3 clusters y sus individuos que los conforman están graficados.

- El grupo 1 (Más fuertes) : Son Pokémon con menor vida y defensa en comparación a los demás grupos pero destacan sobre los demás en su ataque, ataque especial y velocidad.

- El grupo 2 (Equilibrados): De las 6 variables podemos ver como tienen poco ataque y ataque espacial, equilibrado su HP y velocidad y destacan en defensa y defensa especial en comparación a los otros grupos.

- El grupo 3 (Débiles): Notemos que destacan solo en HP, por otro lado tienen poca defensa especial y velocidad.

Con esta interpretación notemos el porque coincide la información del dendrograma y el gráfico de los clusters; los Pokemon de cada cluster destacan ciertos atributos y es la razón de su clustering.


### Conclusiones de Clúster con k-means y Clúster jerárquico.

Notemos que ambos métodos sugieren tres grupos para catalogar a los Pokemons de esta muestra.

El tamaño de integrantes por cluster por metodo son:

| Tamaño | Clúster k-means | Clúster jerárquico |
| --- | ---: | :---: |
|Grupo A (Fuertes)      | 3 | 2 |  
|Grupo B (Equilibrados) | 11 | 13 |
|Grupo C (Debiles)      | 8 | 7 |

Notemos que no hay mucha diferencia entre los grupos que se crearon y de igual forma su análisis por cada método. Por lo que esta forma de catalogarlos es idónea, si quisiéramos un pokemon que tenga muy buenas estadísticas, elegiremos a aquellos del grupo A, si queremos un Pokemon equilibrado podremos elegir uno del grupo B y si quisiéramos uno débil están los del grupo C.

### Mas alla de pokemon.

Aunque Pokemon sea un mundo ficticio rescatemos varias ideas que se toman del mundo real, una de ellas principales es la competencia y ver qué entrenador es más fuerte; en la vida real lo podemos ver en competencias de atletas o caballos, en donde cada uno de los individuos tienen características que los vuelven competitivos, cuantificables y por ende comparables.
